...
module.exports = function (UserIdentity) {
    ...
    function profileToUser(provider, profile, options) {
        ...
        var email = (options.emailAsUserEmail || provider === 'ldap') ? profileEmail : generatedEmail;
        var username = options.emailAsUsername ? profileEmail : provider + '.' + (profile.username || profile.id);
        var password = utils.generateKey('password');
        var userObj = {
            username: username,
            password: password
        };
        if(options.addConstants){
            var addProperties = options.addConstants;
            for (var key in addProperties) {
                userObj[key] = addProperties[key];
            }
        }
        if (email) {
            userObj.email = email;
    ...
    UserIdentity.login = function (provider, authScheme, profile, credentials,
        ...
        userIdentityModel.findOne({
            ...
            var date = new Date();
            userObj.created = date;
            userModel.findOrCreate({where: query}, userObj, function (err, user, created) {
                if (err) {
                    return cb(err);
                }
                if (user && options.addInProfiles && typeof options.addInProfiles === 'object') {
                    var profileField = options.addInProfiles.profileField && (typeof options.addInProfiles.profileField === 'string') ? options.addInProfiles.profileField : undefined;
                    var userUpdate = {};
                    userUpdate["profiles." + provider] = profileField ? profile[profileField] : profile;
                    userUpdate.modified = date;
                    user.updateAttributes(userUpdate, function (err, user) {
                    }); // no action if profile not created in user
                }
